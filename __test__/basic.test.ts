...
describe('open / close', () => {
  ...
  it('rejects opening on a regular file path', async () => {
    ...
    await expect(open(filePath)).rejects.toThrow();
  });

  it('releases the lock on close (reopen works)', async () => {
    const ks1 = await open(dir);
    const p1 = await ks1.partition('p');
    await p1.insert(k('a'), k('1'));
    await ks1.close();

    const ks2 = await open(dir);
...
describe('partition CRUD', () => {
  ...
  it('insert then get returns the value', async () => {
    ...
    const v = p.get(k('hello'));
    expect(v).not.toBeNull();
    expect(v!.toString('utf8')).toBe('world');
  });
  ...
  it('round-trips random byte values up to ~1 MiB', async () => {
    const sizes = [0, 1, 7, 64, 4096, 65536, 1024 * 1024];
    for (const size of sizes) {
      ...
      expect(out!.equals(value)).toBe(true);
    }
  });
});
...
describe('persist', () => {
  it('explicit persist + reopen sees the data', async () => {
    const ks1 = await open(dir);
    const p1 = await ks1.partition('p');
    await p1.insert(k('persist-me'), k('value'));
    await ks1.persist();
    await ks1.close();

    const ks2 = await open(dir);
    const p2 = await ks2.partition('p');
    expect(p2.get(k('persist-me'))!.toString('utf8')).toBe('value');
    await ks2.close();
  });
